 ...
 * @version 1.0
 */
public class SobelLeftRight implements ImageOperation, java.io.Serializable {
    private int radius;


    public SobelLeftRight(){
        this.radius = 1;
    }
        ...
        public BufferedImage apply(BufferedImage input) {
            ...
            for(int x = 0; x < input.getWidth(); x++){
                for(int y = 0; y < input.getHeight(); y++){
                    ...
                    // for a sobel filter we handle negatives by offsetting, so boolean offset is true to execute the first if statement
